...
Aggre_insurance= pd.DataFrame(table1, columns=("States", "Years", "Quarter", "Transaction_typ", 
...
Aggre_transaction= pd.DataFrame(table2, columns=("States", "Years", "Quarter", "Transaction_typ", 
                                                   "Transaction_count", "Transaction_amount"))
...
#Transaction amount count for each Year
...
#Transaction amount and count for each year's Quarter
...
#Function to aggregate transaction type
...
def Aggre_Tran_Transaction_type(df, state):
    ...
    tacyg= tacy.groupby("Transaction_typ")[["Transaction_count","Transaction_amount"]].sum()
    tacyg.reset_index(inplace= True)

    col1,col2= st.columns(2)
    with col1:
        fig_pie_1= px.pie(data_frame= tacyg, names= "Transaction_typ", values= "Transaction_amount",
                            width= 600, title= f"{state.upper()} TRANSACTION AMOUNT", hole= 0.3)
        st.plotly_chart(fig_pie_1)

    with col2:
        fig_pie_2= px.pie(data_frame= tacyg, names= "Transaction_typ", values= "Transaction_count",
...
select = option_menu(None,
                     ...
                     styles={
                         "container": {"width": "1000%", "background": "linear-gradient(to right, #5f8a8b , #5f8a8b )", "padding": "8px 0", "border-radius": "10px", "display": "flex", "justify-content": "space-between"},
                         "icon": {"color": "white", "font-size": "28px", "padding": "10px 20px"},
                         "nav-link": {"font-size": "20px", "text-align": "center", "padding": "10px 20px", "transition": "background-color 0.3s ease", "border-radius": "10px"},
                         "nav-link-selected": {"background-color": "#543B7F"},
                         "nav-link:hover": {"background-color": "#1B5E20", "cursor": "pointer"},
                         "nav-link-first": {"border-top-left-radius": "10px", "border-bottom-left-radius": "10px"},
                         "nav-link-last": {"border-top-right-radius": "10px", "border-bottom-right-radius": "10px"},
                         "separator": {"width": "1px", "background-color": "white", "margin": "0 10px"}
                     })
st.markdown("<h1 style='text-align: center; font-family: Arial;font-size: 3rem; color: white;font-weight: 600; padding: 0 10px;'>PHONEPE PULSE DATA VISUALIZATION AND EXPLORATION: A User-Friendly Tool Using Streamlit and Plotly</h1>", unsafe_allow_html=True)
...
if select == "Home":
    ...
    col1,col2 = st.columns(2)
    with col1:
       #st.image(Image.open(r"D:\Mohanapriya\Proj\pulse\PhonePe-Pulse.jpg"),width=500)
       st.image(Image.open(r"D:\Mohanapriya\Proj\pulse\spotlight_2.png"),width=650)


    with col2:
      
        # Define custom styling using HTML
        custom_style = """
            <style>
                /* Set font to Arial, size to 20px, and margin to 20px */
                p {
        ...
        # Display the styled paragraph
        st.markdown(custom_style, unsafe_allow_html=True)
        st.markdown("<p>The Indian digital payments story has truly captured the world's imagination. From the largest towns to the remotest villages, there is a payments revolution being driven by the penetration of mobile phones and data. </p>", unsafe_allow_html=True)
        st.markdown("<p> When PhonePe started 5 years back, we were constantly looking for definitive data sources on digital payments in India. Some of the questions we were seeking answers to were - How are consumers truly using digital payments? What are the top cases? Are kiranas across Tier 2 and 3 getting a facelift with the penetration of QR codes? his year as we became India's largest digital payments platform with 46% UPI market share, we decided to demystify the what, why and how of digital payments in India. </p>", unsafe_allow_html=True)
        ...
        st.markdown("Inspired from: [Phonepe-Pulse](https://www.phonepe.com/pulse/explore/transaction/2022/4/)")
        st.markdown("<h3 style='color: white; font-family: Forte, sans-serif;'>Crafted by: Mohana Priya T V</h3>", unsafe_allow_html=True)
        st.markdown("<p style='font-family: Forte, sans-serif; font-size: larger;'><a href='https://github.com/beingbvh' style='text-decoration: none; color: #FFFFFF; font-family: Forte, sans-serif;'>Github</a> &nbsp; | &nbsp; <a href='https://www.linkedin.com/in/mohana-priya-t-v-810200202/' style='text-decoration: none; color: #FFFFFF; font-family: Forte, sans-serif;'>LinkedIn</a></p>", unsafe_allow_html=True)
        
        st.write("---")
        

elif select == "Explore Data":
    image = Image.open(r"D:/Mohanapriya/Proj/pulse/Pulse2.jpg")
    st.image(image, width=900, caption='Pulse2.jpg', use_column_width=True, output_format='auto')
    createtab = option_menu(None,
                       ...
                       styles={"container": {"width": "100%"},
                               ...
                               "nav-link-selected": {"background-color": "#5263cb"}})
    if createtab == "Aggregated Analysis":
        
        #method = st.radio("Select The Method",["Insurance Analysis", "Transaction Analysis", "User Analysis"])
        selectmethod = option_menu(None,
                       options = ["Insurance","Transactions","Users",],
        ...
        if selectmethod == "Insurance":

            col1,col2= st.columns(2)
            with col1:
            
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
            years= st.slider("",Aggre_insurance["Years"].min(), Aggre_insurance["Years"].max(),Aggre_insurance["Years"].min())
            ...
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Quarter</span>", unsafe_allow_html=True)
                quarters= st.slider("Select Quarter",tac_Y["Quarter"].min(), tac_Y["Quarter"].max(),tac_Y["Quarter"].min())
            Transaction_amount_count_Y_Q(tac_Y, quarters)

        elif selectmethod == "Transactions":
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
            ...
            col1,col2= st.columns(2)
            with col1:
                states= st.selectbox("Select State", Aggre_tran_tac_Y["States"].unique())
            ...
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Quarter</span>", unsafe_allow_html=True)
                quarters= st.slider("Select Quarter",Aggre_tran_tac_Y["Quarter"].min(), Aggre_tran_tac_Y["Quarter"].max(),Aggre_tran_tac_Y["Quarter"].min())
            Aggre_tran_tac_Y_Q= Transaction_amount_count_Y_Q(Aggre_tran_tac_Y, quarters)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State_wise Transations", Aggre_tran_tac_Y_Q["States"].unique())

            Aggre_Tran_Transaction_type(Aggre_tran_tac_Y_Q, states)

            
        elif selectmethod == "Users":
            ...
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State", Aggre_user_Y_Q["States"].unique())

            Aggre_user_plot_3(Aggre_user_Y_Q, states)

    if createtab == "Map Analysis":

        #method_2= st.radio("Select Mapping Method",["Map Insurance", "Map Transaction", "Map User"])
        selectmethod_Map = option_menu(None,
                      ...
                      styles={"container": {"width": "100%"},
                              "icon": {"color": "white", "font-size": "24px"},
                              "nav-link": {"font-size": "24px", "text-align": "center", "margin": "-2px"},
                              "nav-link-selected": {"background-color": "#f78c3d"}})
        

        if selectmethod_Map == "Map Insurance":
        
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
                years= st.slider("Select Year for Mapping Insurances",map_insurance["Years"].min(), map_insurance["Years"].max(),map_insurance["Years"].min())
            map_insur_tac_Y= Transaction_amount_count_Y(map_insurance, years)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State for Mapping Insurances", map_insur_tac_Y["States"].unique())
            ...
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Quarter</span>", unsafe_allow_html=True)
                quarters= st.slider("Select Quarter for Mapping Insurances",map_insur_tac_Y["Quarter"].min(), map_insur_tac_Y["Quarter"].max(),map_insur_tac_Y["Quarter"].min())
            map_insur_tac_Y_Q= Transaction_amount_count_Y_Q(map_insur_tac_Y, quarters)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State to see overall Insurances", map_insur_tac_Y_Q["States"].unique())

            Map_insur_District(map_insur_tac_Y_Q, states)


        elif selectmethod_Map == "Map Transaction":
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
                years= st.slider("Select Year for Mapping Transactions",map_transaction["Years"].min(), map_transaction["Years"].max(),map_transaction["Years"].min())
            map_tran_tac_Y= Transaction_amount_count_Y(map_transaction, years)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State for Mapping Transactions", map_tran_tac_Y["States"].unique())
            ...
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Quarter</span>", unsafe_allow_html=True)
                quarters= st.slider("Select Quarter for Mapping Transactions",map_tran_tac_Y["Quarter"].min(), map_tran_tac_Y["Quarter"].max(),map_tran_tac_Y["Quarter"].min())
            map_tran_tac_Y_Q= Transaction_amount_count_Y_Q(map_tran_tac_Y, quarters)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State for Mapping the district wise Transactions", map_tran_tac_Y_Q["States"].unique())

            Map_insur_District(map_tran_tac_Y_Q, states)



        elif selectmethod_Map == "Map User":
            
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
                years= st.slider("Select Year for Mapping User",map_user["Years"].min(), map_user["Years"].max(),map_user["Years"].min())
            map_user_Y= map_user_plot_1(map_user, years)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Quarter</span>", unsafe_allow_html=True)
                quarters= st.slider("Select Quarter for Mapping User",map_user_Y["Quarter"].min(), map_user_Y["Quarter"].max(),map_user_Y["Quarter"].min())
            map_user_Y_Q= map_user_plot_2(map_user_Y, quarters)

            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select State</span>", unsafe_allow_html=True)
                states= st.selectbox("Select State for Mapping User", map_user_Y_Q["States"].unique())

            map_user_plot_3(map_user_Y_Q, states)


    if createtab == "Top Analysis":

        #method_3= st.radio("Select The Method",["Top Insurance", "Top Transaction", "Top User"])
        selectmethod_Top = option_menu(None,
                       ...
                       styles={"container": {"width": "100%"},
                               "icon": {"font-size": "24px"},
                               "nav-link": {"font-size": "24px", "text-align": "center", "margin": "-2px"},
                               "nav-link-selected": {"background-color": "#254b46"}})

        if selectmethod_Top == "Top Insurance":
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
        ...
        elif selectmethod_Top == "Top Transaction":
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
        ...
        elif selectmethod_Top == "Top User":
            col1,col2= st.columns(2)
            with col1:
                st.markdown("<span style='color:white; font-size:25px;'>Select Year</span>", unsafe_allow_html=True)
    ...
    # st.markdown("<span style='color:white; font-size:25px;'>Please select a question from the dropdown menu</span>", unsafe_allow_html=True)
    ...
    st.markdown("<style>div[role='listbox'] div[role='option'] { font-family: Arial; font-size: 16px; }</style>", unsafe_allow_html=True)
    st.markdown("<span style='color:white; font-size:25px;'>Please select a question from the dropdown menu</span>", unsafe_allow_html=True)
    ...
    if question == "1. Transaction Amount and Count of Aggregated Insurance":
        ...
        st.markdown("<span style='color:white; font-size:25px;'>TRANSACTION AMOUNT</span>", unsafe_allow_html=True)
        st.subheader("")
